...
function App() {
  ...
  const handleAddFoodItem = (name, serving, protein, calories, sugar) => { //get a meal and add to mealItems
    ...
    // food_data.push(food);
    setNewFoodItems([...setNewFoodItems, food]); //'
    localStorage.setItem('setNewFoodItems', JSON.stringify(setNewFoodItems));

    console.log(newFoodItems)
  };

  useEffect(() => {
    setNewFoodItems(localStorage.getItem('setNewFoodItems') ? JSON.parse(localStorage.getItem('setNewFoodItems'))
      : []
    );
  }, []);
