...
function App() {
  ...
  const { food_data, setFood_Data } = udata;
  const [newFoodItems, setNewFoodItems] = useState(food_data); //totalNutrients is [] empty array
  ...
  const handleAddFoodItem = (name, serving, protein, calories, sugar) => { //get a meal and add to mealItems
    ...
    localStorage.setItem('newFoodItems', JSON.stringify(newFoodItems));
  ...
  useEffect(() => {
    setFood_data(localStorage.getItem('newFoodData') ? JSON.parse(localStorage.getItem('newFoodData'))
      : []
    );
  }, []);
        ...
        <div className="row">
          ...
          <div className="col-left">
            <div className="wrapper-card-food-list">
              ...
              />
              <FoodList
                food_data={newFoodItems.filter((food) => food.name.toLowerCase().includes(searchText))}
