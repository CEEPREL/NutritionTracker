...
function App() {
  ...
  const [ food_data, setFoodData ] = food_data_json;
  ...
  const handleAddFoodItem = (name, serving, protein, calories, sugar) => { //get a meal and add to mealItems
    ...
    // food_data.push(food);
    const newFoods = [...food_data, food]
    setFoodData(newFoods); //'
    localStorage.setItem('food_data', JSON.stringify(newFoods));

    console.log(newFoodItems)
  };

  useEffect(() => {
    setFoods(localStorage.getItem('food_data') ? JSON.parse(localStorage.getItem('food_data'))
      : []
    );
  }, []);
