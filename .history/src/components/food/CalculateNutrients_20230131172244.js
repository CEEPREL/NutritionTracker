...
export default function CalculateNutrients(
  ...
  const totalProtein = Math.round(mealItems.reduce((acc, item) => acc + item.qty * item.protein, 0));
  const totalServing = Math.round(mealItems.reduce((a, item) => a + item.qty * item.serving, 0));
  const totalCalories = Math.round(mealItems.reduce((a, item) => a + item.qty * item.calories, 0));
  const totalSugar = Math.round(mealItems.reduce((a, item) => a + item.qty * item.sugar, 0));
          ...
          <div>
            ...
            {mealItems.map((item) =>
              <div key={item.id} className="totals-row">
                ...
                <div className="hoverText totals-col-3 hoverText" style={{ textAlign: 'left', whiteSpace:'noWrap', overflow:'hidden', textOverflow:'ellipsis', :hover:'whiteSpace:'noWrap''}}><small>x{item.qty}</small> {item.name}</div>
            ...
            <div className="total-food-nutrients">
              ...
              <p>Protein <b>{remainingProtein < 0 ? 0 : remainingProtein}g</b></p>
              <p>Calories <b>{remainingCalories < 0 ? 0 : remainingCalories}c</b></p>
              <p>Sugar <b>{remainingSugar < 0 ? 0 : remainingSugar}g</b></p>
            </div>
