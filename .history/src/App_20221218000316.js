...
function App() {
  ...
  const [ food_dat, setFoodData ] = food_data;
  const [newFoodItems, setNewFoodItems] = useState(food_dat); //totalNutrients is [] empty array
  ...
  const handleAddFoodItem = (name, serving, protein, calories, sugar) => { //get a meal and add to mealItems
    ...
    // food_data.push(food);
    const newFoods = [...food_dat, food]
    setFoodData(newFoods); //'
    localStorage.setItem('food_dat', JSON.stringify(newFoods));

    console.log(newFoodItems)
  };

  useEffect(() => {
    setFoodData(localStorage.getItem('food_dat') ? JSON.parse(localStorage.getItem('food_dat'))
      : []
    );
  }, []);
        ...
        <div className="row">
          ...
          <div className="col-left">
            <div className="wrapper-card-food-list">
              ...
              />
              <FoodList
                food_data={food_dat.filter((food) => food.name.toLowerCase().includes(searchText))}
